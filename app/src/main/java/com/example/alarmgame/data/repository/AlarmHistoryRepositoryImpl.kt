...
class AlarmHistoryRepositoryImpl @Inject constructor(
    ...
    override suspend fun insert(history: AlarmHistory): Long =
        historyDao.insert(history.toEntity())

    override fun observeByAlarm(alarmId: Long, limit: Int): Flow<List<AlarmHistory>> =
        historyDao.observeByAlarm(alarmId, limit).map { entities ->
            entities.map { it.toDomain() }
        }
}

private fun AlarmHistoryEntity.toDomain(): AlarmHistory = AlarmHistory(
    id = id,
    alarmId = alarmId,
    firedAt = firedAt,
...
private fun AlarmHistory.toEntity(): AlarmHistoryEntity = AlarmHistoryEntity(
    id = id,
    alarmId = alarmId,
    firedAt = firedAt,
    dismissedAt = dismissedAt,
